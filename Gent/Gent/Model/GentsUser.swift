...
class GentsUser: NSObject {
    ...
    func registerUser(withName: String, email: String, password: String, cardSourceID: String? = nil, userData:[String:String], completion: @escaping completionSuccess) {
        ...
        GentsUser.firebaseGentsAuth()?.createUser(withEmail: email, password: password) { (user, error) in
            ...
            StripeAPIClient.sharedClient.createStripeCustomer(email: email, cardSourceID: cardSourceID) { resp, err in
